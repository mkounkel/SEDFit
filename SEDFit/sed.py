...
class SEDFit:
    def __init__(self,ra=' ',dec=' ',radius=1,frame='icrs',filename='',write=True,vizier_filename=None,parallax_sigma=3,
        ...
        if (ra!=' ') & (dec!=' '):
            ...
            'WISE.W1', 'WISE.W2', 'WISE.W3','Spitzer.IRAC.3.6','Spitzer.IRAC.4.5','Spitzer.IRAC.5.8','Spitzer.IRAC.8.0',
            'Spitzer.MIPS.24','XMM-OT.V','XMM-OT.B','XMM-OT.U','XMM-OT.UVW1','XMM-OT.UVM2','XMM-OT.UVW2'])
            self.sed['la']=np.round(np.array([1538.6207014101276, 2315.6631043370066, 3531.050228310502, 3561.7887353001856,
            ...
            34002.551861150176, 46520.081762929745, 128075.76485642244,
            35500,44930,57310,78720,236700,5430,4506,3441,2908,2311,2119]),1)*u.AA
            self.sed['width']=np.round((np.array([0.0269,0.0616,0.0619,0.0555,0.0639,0.0928,
                                        ...
                                        0.66,1.04,5.51,0.749,1.02,1.421,2.881,9.13,
                                        0.085,0.1095,0.086,0.1155,0.0705,0.0649])/2*u.micron).to(u.AA),1)
            self.sed['flux']=np.nan*u.erg/u.s/(u.cm**2)/u.AA
            self.sed['eflux']=np.nan*u.erg/u.s/(u.cm**2)/u.AA

            self.sed=self.sed[np.argsort(self.sed['la'])]
            self.maxav=10
